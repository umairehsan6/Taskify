...
if (projectTasksModal) {
    projectTasksModal.addEventListener('show.bs.modal', function (event) {
        ...
        fetch(`/get-project-tasks/${projectId}/`, {
            method: 'GET',
            headers: {
                            ...
                            <div class="task-card card p-3" data-priority="${task.priority}" data-task-id="${task.id}">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div class="flex-grow-1">
                                        ...
                                        ${task.report ? `<p class=" mt-2 mb-0">Report: ${task.report}</p>` : ''}
                                        ${task.file_url ? 
                                        `<p class=" mt-2 mb-0">
                                            File Url: 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (viewFilesModal) {
        viewFilesModal.addEventListener('show.bs.modal', function(event) {
            ...
            const baseUrl = '/upload_task_file/0/';
            ...
            if (reportForm) {
                reportForm.action = `/upload_task_report/${taskId}/`;
...
if (loginForm) {
    ...
    window.testProjectNotification = function(projectId) {
        ...
        fetch(`/test-project-notification/${projectId}/`, {
            method: 'GET',
            headers: {
                'X-CSRFToken': getCookie('csrftoken')
            }
        ...
        container.innerHTML += `
            <div class="task-card card p-3 mb-3" data-priority="${task.priority.toLowerCase()}">
                ...
                <div class="task-card-buttons2 d-flex gap-2 mt-2">
                    <form method="POST" action="/start-task/${task.id}/" class="start-task-form">
    ...
    fetch('/ongoing-tasks/')
        ...
        .then(data => {
            ...
            data.tasks.forEach(task => {
                ...
                // console.log('Opening modal for taskId:', taskId);
                let actionButtons = `
                    <form method="POST" action="/complete-task/${task.id}/" class="complete-task-form">
                        <input type="hidden" name="csrfmiddlewaretoken" value="${getCookie('csrftoken')}">
                ...
                if (task.status === 'On Hold') {
                    actionButtons += `
                        <form method="POST" action="/start-working/${task.id}/" class="start-working-form">
                            <input type="hidden" name="csrfmiddlewaretoken" value="${getCookie('csrftoken')}">
                    ...
                    actionButtons += `
                        <form method="POST" action="/stop-working/${task.id}/" class="stop-working-form">
                            <input type="hidden" name="csrfmiddlewaretoken" value="${getCookie('csrftoken')}">
                ...
                container.innerHTML += `
                    <div class="ongoing-task-item card p-3 mb-3" data-priority="${task.priority.toLowerCase()}">
                        ...
                        <p class="mb-1">Description: <strong>${task.description}</strong></p>
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (modal) {
                        ...
                        if (fileDisplay) {
                            fileDisplay.innerHTML = '<div class="mb-2">Loading file...</div>';
                            fetch(`/get-task-file/${formData.get('task_id')}/`)
                                .then(response => response.json())
                                .then(data => {
                                    if (data.success && data.file_url) {
                                        fileDisplay.innerHTML = `
...
document.addEventListener('click', function(e) {
    ...
    if (btn) {
        ...
        if (viewFilesModal) {
            ...
            const fileForm = viewFilesModal.querySelector('.uploadFileForm');
            if (fileForm) {
                fileForm.action = `/upload_task_file/${taskId}/`;
            ...
            <div class="task-card card p-3 mb-3" data-priority="high" data-task-id="${task.id}" data-project-id="${task.project_id || ''}">
                ...
                <div class="task-card-buttons2 d-flex gap-2 mt-2">
                    ${
                      ...
                      ? `
                        <form method="POST" action="/approve-task/${task.id}/" class="approve-task-form">
                            <input type="hidden" name="csrfmiddlewaretoken" value="${getCookie('csrftoken')}">
...
function pollApprovalPendingTasks() {
    fetch('/approval-pending-tasks-json/', {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
...
function updateTodayStats() {
    ...
    fetch('/employee-today-stats-json/', {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
                    ...
                    <div class="col-12 mb-3">
                        <div class="task-card card p-3" data-priority="${task.priority}" data-task-id="${task.id}">
                            <div class="d-flex justify-content-between align-items-start">
                                <div class="flex-grow-1">
                                    <h6 class="fw-semibold mb-2">${task.task_name}</h6>
                                    <p class="text-white mb-2">${task.task_description}</p>
                                    <div class="d-flex gap-2 mb-2">
                                        <span class="badge 
                                            ${task.status === 'in_progress' ? 'bg-success' : 
                                              task.status === 'completed' ? 'bg-primary' : 
...
// ... existing code ...
    ...
    async function fetchNotifications() {
        try {
            const response = await fetch('/employee-notifications-json/');
...
function deleteTask(taskId, projectId, cardElem) {
    fetch(`/delete-task/${projectId}/?task_id=${taskId}`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken'),
            'X-Requested-With': 'XMLHttpRequest'
...
// Wait for the page to be fully loaded, including all scripts
    window.addEventListener('load', function() {
        console.log('Page fully loaded - marking notifications as read in backend only');
        
        // Add a small delay to ensure everything is ready
        setTimeout(function() {
            console.log('Making fetch request to mark notifications as read (UI will not change)');
            
            // Silently mark notifications as read in the backend
            ...
            .then(data => {
                ...
                if (data.success) {
                    console.log('Success! Notifications marked as read in backend. UI unchanged until page reload.');
                } else {
                    console.log('Failed to mark notifications as read:', data);
                }
            })
            .catch(error => {
