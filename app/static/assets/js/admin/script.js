...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (viewFilesModal) {
    viewFilesModal.addEventListener("show.bs.modal", function (event) {
      ...
      if (uploadFileForm) {
        uploadFileForm.action = `/upload_task_file/${taskId}/`;
      ...
      if (fileUrl && fileUrl !== "#") {
        fileDisplay.innerHTML = `
          <div class="mb-2">
            <a href="/download_task_file/${taskId}/" class="btn btn-outline-primary btn-sm">
  ...
  window.filterTasksByProject = function () {
    ...
    tasksTableBody.innerHTML = "";
    fetch("/tasks/get-tasks/?project_id=" + projectId)
      .then((response) => response.json())
      .then((data) => {
        if (data.tasks.length > 0) {
          tasksTableContainer.classList.remove("d-none");
...
document.querySelectorAll('.update-status-btn').forEach(function(button) {
  button.addEventListener('click', function(e) {
      e.preventDefault();
      const projectId = this.getAttribute('data-project-id');
      const newStatus = parseInt(this.getAttribute('data-new-status'));
      const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]')?.value;
      fetch(`/update-project-status/${projectId}/${newStatus}/`, {
          method: 'POST',
          headers: {
              'X-Requested-With': 'XMLHttpRequest',
              'X-CSRFToken': csrfToken
    ...
    document.querySelectorAll('.assign-task-link').forEach(function(link) {
        link.addEventListener('click', function(e) {
            ...
            if (employeeSelect) {
                ...
                fetch(`/get-project-employees/${projectId}/`)
                    .then(response => response.json())
                    .then(data => {
    ...
    fetch(getTasksUrl + '?project_id=' + projectId)
        ...
        .then(data => {
            if (data.tasks.length > 0) {
                ...
                const statusMap = {
                    'pending_approval': [5], // only Not Assigned
                    'ongoing': [1],          // only In Progress
                    'on_hold': [3],          // only On Hold
                    'pending': [0],          // only Pending
                    'completed': [2]         // only Completed
                };
                ...
                filteredTasks.forEach((task, index) => {
                    const statusClass = task.status === 2 ? 'bg-success' :
                                      task.status === 1 ? 'bg-primary' :
                                      task.status === 3 ? 'bg-warning' : 'bg-secondary';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    userTable.addEventListener('click', function(e) {
        ...
        if (assignBtn) {
            ...
            const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]')?.value;
            fetch(`/assign-department/${userId}/${deptId}/`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'X-CSRFToken': csrfToken
...
function renderAdminNotifications(notifications) {
    ...
    if (!notifications.length) {
        list.innerHTML = '<li class="p-4 text-center" style="min-height: 60px;">No notifications</li>';
...
//admin notification handling 
    window.addEventListener('load', function() {
        setTimeout(function() {
            ...
            .then(response => response.json())
            .then(data => {
                // Optionally handle response, but do NOT update the UI instantly
            })
            .catch(error => {
